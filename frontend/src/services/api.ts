...
export async function getRAsByCourse(courseId: string): Promise<RA[]> {
  ...
  return rows.map((it) => {
    ...
    return {
      id,
      titulo,
      // No mostrar porcentaje en el subtítulo del RA
...
export async function getCourseGradeSummary(courseCode: string, studentId: string): Promise<GradeSummaryResponse | null> {
  try {
    const { data } = await api.get(endpoints.asignaturas.calificaciones(courseCode, studentId))
    // Validar forma mínima
    if (data && typeof data === 'object' && data.asignatura && data.total) {
      return data as GradeSummaryResponse
    }
    return null
  } catch (e) {
    if (import.meta.env.DEV) console.debug('getCourseGradeSummary failed', e)
    return null
  }
}
...
// Actualizar una RA-Actividad (actividad y/o relación con el RA)
export async function updateRaActividad(raId: string, raActividadId: string, payload: {
  nombre_actividad?: string
  descripcion?: string
  porcentaje_ra_actividad?: number
  ...
  const { data } = await api.patch(endpoints.ras.actividad(raId, raActividadId), payload)
  return data
}

// Eliminar RA-Actividad (requiere contraseña del docente)
export async function deleteRaActividad(raId: string, raActividadId: string, password: string) {
  const { data, status } = await api.delete(endpoints.ras.actividad(raId, raActividadId), {
    data: { password },
  })
  return { ok: status === 204, data }
}

// Eliminar Indicador de Logro (requiere contraseña del docente)
export async function deleteIndicador(raId: string, indicadorId: string, password: string) {
  const { data, status } = await api.delete(endpoints.ras.indicador(raId, indicadorId), {
    data: { password },
  })
  return { ok: status === 204, data }
}
