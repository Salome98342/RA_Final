...
import { createActivityMulti, getRAsByCourse, getTiposActividad, getIndicatorsByRA } from '@/services/api'
...
const NuevaActividadCurso: React.FC = () => {
  ...
  const [raIndSel, setRaIndSel] = useState<Record<string, string[]>>({})
  ...
  // Validación de totales por RA basada en peso interno eliminada
  ...
  const submit = async () => {
    ...
    if (form.cierre) {
      ...
      if (form.cierre < today) { setToast({ text: `La fecha límite no puede ser anterior a hoy (${today}).`, type: 'error' }); return }
    }
    // Prechequeo de peso interno eliminado
        ...
        <main className="dash-content">
          ...
          <div className="row g-3">
            <div className="col-md-6">
              ...
              <input id="nombreAct" className="form-control" placeholder="Ej. Taller 1" value={form.nombre} onChange={e=>setForm(f=>({...f, nombre:e.target.value}))} />
            </div>
            {/* Campo de peso interno eliminado */}
            <div className="col-md-3">
              <label className="ra-small d-block mb-1" htmlFor="tipoAct">Tipo de actividad</label>
              <select id="tipoAct" className="form-select" value={form.tipo} onChange={e=>setForm(f=>({...f, tipo:e.target.value}))}>
                {tipos.length===0 ? <option value="">Selecciona…</option> : tipos.map(t => <option key={t.id} value={t.id}>{t.descripcion}</option>)}
              </select>
            </div>

            <div className="col-md-4">
              <label className="ra-small d-block mb-1" htmlFor="fechaCierre">Fecha límite</label>
              <input id="fechaCierre" className="form-control" type="date" min={new Date().toISOString().slice(0,10)} value={form.cierre} onChange={e=>setForm(f=>({...f, cierre:e.target.value}))} />
            </div>
            <div className="col-12">
              <label className="ra-small d-block mb-1" htmlFor="descAct">Descripción (opcional)</label>
              <textarea id="descAct" className="form-control" placeholder="Instrucciones, criterios, etc." value={form.desc} onChange={e=>setForm(f=>({...f, desc:e.target.value}))} />
            </div>
            ...
            <div className="col-12">
                  ...
                  <table className="table table-sm align-middle">
                    <thead>
                      <tr>
                        ...
                        <th>RA</th>
                        <th>Aporte al RA (%)</th>
                        {/* Columnas de totales y quedaría eliminadas */}
                        <th>Indicadores</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <>
                            <tr key={r.id}>
                              ...
                              <td>{r.titulo}</td>
                              <td>
                                <input className="form-control form-control-sm" type="number" step="0.01" min={0} max={100} placeholder="% RA" value={aporteRA[r.id] ?? ''} onChange={(e)=> setAporteRA(m=>({ ...m, [r.id]: e.target.value }))} disabled={!checked} />
                              </td>
                              <td>
                                <button className="btn btn-outline-secondary btn-sm" disabled={!checked} onClick={()=> toggleIndicators(r.id)}>
              ...
              <div className="form-text">Regla: el aporte total de actividades a cada RA puede llegar hasta 100%.</div>
            </div>

            <div className="col-12 d-flex gap-2">
              <button className="btn btn-danger" disabled={!canSave || saving} onClick={submit}>{saving? 'Guardando…' : 'Crear actividad'}</button>
              <button className="btn btn-outline-secondary" onClick={()=>navigate(`/docente/${curso}/ras`)}>Cancelar</button>
            </div>
          </div>
        </main>
      </div>
